...
void inputData(vector<vector<double>>& A, vector<double>& b, vector<double>& c) {
    ...
    if (!(cin >> n)) {
        cerr << "Ошибка ввода: количество переменных должно быть целым числом." << endl;
        exit(1);
    }
    cout << "Введите количество ограничений: ";
    if (!(cin >> m)) {
        cerr << "Ошибка ввода: количество ограничений должно быть целым числом." << endl;
        exit(1);
    }
    ...
    for (int j = 0; j < n; ++j) {
        if (!(cin >> c[j])) {
            cerr << "Ошибка ввода: коэффициенты целевой функции должны быть числами." << endl;
            exit(1);
        }
    }
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            if (!(cin >> A[i][j])) {
                cerr << "Ошибка ввода: коэффициенты ограничений должны быть числами." << endl;
                exit(1);
            }
        }
    }
    ...
    for (int i = 0; i < m; ++i) {
        if (!(cin >> b[i])) {
            cerr << "Ошибка ввода: правые части ограничений должны быть числами." << endl;
            exit(1);
        }
    }
